...
**Inbox Cleaner (standalone process — designed 2026-07-04, code complete 2026-07-05; supersedes the earlier "Paul Inbox Review" two-pass design):** Triages colleagues' unorganized inboxes at 200,000+ message scale, then becomes a permanent, per-user recurring process. First user: Matt (`--inbox-matt`; switched from Paul 2026-07-05 — Paul becomes a later config block); designed multi-user from day one — each user is a config block plus IT adding their mailbox to the Application Access Policy (app-token read, same mechanism as `--email-brain`; no Exchange delegation needed). **Implementation:** `inbox_cleaner.py` (all five stages + daily Claude rules update) and `teams.py` (delegated Teams chat transport, shared with the future Remy-via-Teams feature), dispatched as `rocky.py --inbox-<user> --snapshot|--analyze|--chat|--rules-update|--execute|--status`. Operational guide, IT steps, and the onboarding questionnaire: `INBOX_CLEANER.md` + `_templates/inbox_questionnaire.md`.
...
**James's own process — REMOVED 2026-08-29.** `--inbox-james` (built 2026-07-12/13) ran the shared machinery in a small-inbox mode: a `--cycle` one-shot (snapshot → analyze → chat → execute-approved; dashboard "James Inbox" button), the "sort with friends" conversation-sort pass (his SortByConversation Outlook VBA macro ported to Graph), open chat mode with action proposals (`chat_mode: "open"`), and the Engineer one-email workup. All of it — the code in `inbox_cleaner.py`, the `inbox_users.james` config block, and the dashboard entry — was removed 2026-08-29. Matt's strict-protocol deep-clean process is unaffected; the generic `exclude_senders`/`exclude_domains` support in sender_routes.json remains (now hand-edited only). History: the 2026-07-12/13 SESSIONS.md entries and the git log.

**Open items (post-build, 2026-07-05):** Claude batch pass over the ambiguous residual (designed, not wired — v1 leaves unmatched mail alone); reminders/timeouts for unanswered Teams proposals; dashboard registry entries for Matt's stage-by-stage `inbox-*` commands; Adaptive Cards (v1 is plain-text one-at-a-time proposals, which keeps "yes" unambiguous); maintenance cadence per user (default weekly). Resolved during build: CLI is `--inbox-<user>` (processes sort together: inbox-matt, inbox-paul); rules file is plain-English markdown with `[C####] [approved/declined YYYY-MM-DD]` provenance tags, machine-executable via cohorts.json; questionnaire is 10 questions in `_templates/inbox_questionnaire.md`, sent **by email** from rocky@ as HTML with an answer box under each question (`Answer [iq-N]:` markers; Rocky watches her inbox for the reply and parses the boxes, Maple-digest style) — Teams carries only the one-at-a-time cohort proposals.
...
- A long-running host loop already exists (`--monitor`, which runs its commands as fresh subprocesses each cycle), a natural place to also poll for Teams replies. (The original 24/7 `--monitor-remy` loop was retired 2026-08-29 — unused, and a boot-launched process runs a stale exe across rebuilds; `classify_email` and `remy_runner.py` remain in place for the re-wire.)
...
**Decided (2026-08-30):**
- **Invocation source: the Remy codebase, never an exe.** Rocky runs Remy from the local GitHub checkout (`remy_cli_path` → `remy_cli.py` under a Python interpreter, per `remy_runner.py`). A `git pull` in the REMY checkout updates headless Remy immediately with no Rocky rebuild. Keep it this way; do not point `remy_cli_path` at a frozen exe.
- **"What to ask" lives in Remy; "how to ask" lives in Rocky.** When Rocky queries the attorney (Teams/email) for the inputs a request needs, the determination of *which* inputs comes from Remy: a new `describe` subcommand on `remy_cli.py` emits a machine-readable manifest per workflow — required fields (labels/types), required attachments, valid forms per jurisdiction, and whether the workflow supports the extract → review → generate split. Rocky's flow: classifier picks the category → query Remy's manifest → render the question/card from it → collect answers → invoke. The extraction-review step likewise renders from Remy's `--extract-only` JSON. Rationale: Rocky's current hardcoded duplicates (`CATEGORY_TO_CLI`, `REQUIRED_ATTACHMENTS`, `_DEFAULT_FORM_TYPE`, the arg builders) drift as Remy evolves — the same staleness class that retired `--monitor-remy`, at the data layer. Rocky keeps the classifier, transport, pending-job state machine, attorney routing, timeouts, and the generic manifest→card renderer; Remy emits structure, Rocky emits English.
